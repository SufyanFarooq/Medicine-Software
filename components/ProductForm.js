...
export default function ProductForm({ product = null, isEditing = false }) {
  ...
  useEffect(() => {
    ...
    // Fetch existing products for validation (only for new products)
    if (!isEditing) {
  ...
  const handleStockUpdate = async () => {
    ...
    if (!product || !product._id) {
      setError('Product ID not found. Please refresh the page and try again.');
      return;
    }

    // Check if we have the required form data
    if (!formData.quantity || !formData.purchasePrice) {
      setError('Current product data is incomplete. Please refresh the page and try again.');
      return;
    }
    ...
    try {
      ...
      // Also update the product document to include a transaction history
      try {
        const productUpdateResponse = await apiRequest(`/api/products/${product._id}`, {
          method: 'PUT',
          body: JSON.stringify({
            ...formData,
        ...
        if (!productUpdateResponse.ok) {
          console.error('Failed to update product:', await productUpdateResponse.json());
        }
      } catch (updateError) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        // Log activity
        if (isEditing) {
          logProductActivity.updated(formData.name, formData.code);
        } else {
          logProductActivity.added(formData.name, formData.code);
          
          // For new products, create initial inflow transaction
          try {
            const productData = await response.json();
            console.log('New product created, creating initial inflow transaction...');
